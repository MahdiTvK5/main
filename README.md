...
### حالت تعاملی `deploy.sh`: پروژه اصلی یا یک برنچ؟

با اجرای `bash deploy.sh` اول پرسیده می‌شود چه چیزی را نصب/به‌روزرسانی کنیم:

* **`[m]` پروژه اصلی** ← همان مسیر جاری، سرویس `overwallbot` (رفتار کلاسیک پروداکشن؛ کد از `origin/main` ریست می‌شود و `.env` دست نمی‌خورد).
* **`[b]` یک برنچ گیت** ← ابتدا لیست همه برنچ‌های روی origin (به‌جز main/master) با شماره نمایش داده می‌شود و برنچ موردنظر را با شماره انتخاب می‌کنید. سپس آن برنچ در یک پوشه‌ی کاملاً جدا یعنی `../overwallbot-<نام‌برنچ>` نصب می‌شود با:
  * venv و فایل `.env` مستقل خودش،
  * دیتابیس جداگانه (`overwall_<نام‌برنچ>`) — به داده‌ی پروداکشن دست نمی‌زند،
  * پورت وب‌پنل متفاوت (به‌صورت خودکار اولین پورت آزاد بعدی)،
  * سرویس systemd مستقل (`overwallbot-<نام‌برنچ>`).

بنابراین نصب برنچ هیچ تداخلی با پروژه اصلی ندارد. توجه: برای هر برنچ بهتر است **توکن ربات جداگانه** بدهید؛ دو نمونه با یک توکن «Conflict» می‌گیرند.

**Health Check:** پایان هر دو مسیر (اصلی/برنچ) با تست سلامت همراه است؛ اسکریپت تا حدود ۶۰ ثانیه منتظر می‌ماند سرویس active شود و لاگ استارت موفق («ربات با موفقیت استارت شد...») در journal ثبت گردد. اگر شکست بخورد، ۶۰ خط آخر لاگ و چک‌لیست علت‌های رایج نمایش داده می‌شود.
